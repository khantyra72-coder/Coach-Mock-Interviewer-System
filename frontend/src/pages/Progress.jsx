...
function declaredInterviewTypes(session) {
  const values = Array.isArray(session.types) ? session.types : String(session.type || '').split(',')
  return values.map((value) => value.trim()).filter(Boolean)
}

function scoreForInterviewType(session, type) {
  const categoryScore = session.metrics?.[TYPE_METRIC_KEYS[type]]
  if (Number.isFinite(categoryScore)) return categoryScore
...
function ProgressBar({ icon: Icon, label, value, count }) {
  return <div className="progress-bar-row"><span className="progress-row-icon"><Icon size={20} strokeWidth={1.8} /></span><span className="progress-row-label">{label}{count ? <small>{count} session{count === 1 ? '' : 's'}</small> : null}</span><span className="progress-bar-track"><i style={{ width: `${value ?? 0}%` }} /></span><strong>{value === null ? 'Not practiced' : `${value}%`}</strong></div>
}
...
export default function Progress() {
    ...
    async function loadProgress() {
      try {
        ...
        const localOnly = getUnsyncedLocalSessions()
          ...
          .map((session) => ({ id: session.id, completedAt: session.completedAt, role: session.role || 'Not specified', type: session.type || 'Technical', types: session.types, score: session.score ?? session.result?.overallScore ?? 0, metrics: session.result }))
  ...
  const typeScores = INTERVIEW_TYPES.map((type) => {
    const scores = sessions.map((session) => scoreForInterviewType(session, type)).filter(Number.isFinite)
    return { label: type, value: scores.length ? Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length) : null, count: scores.length, icon: type === 'Technical' ? Code2 : type === 'Behavioral' ? MessageCircle : Layers3 }
  })
